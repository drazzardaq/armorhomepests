    ...
    >
      ...
      >
        <div class="relative aspect-[16/9]">
          <img 
            :src="`/images/ai/${slide.image}`" 
            :alt="slide.title"
            class="w-full h-full object-cover"
          >
          <div v-if="slide.title || slide.description" class="absolute inset-0 bg-gradient-to-t from-dark-gray/80 to-transparent"></div>
          <div v-if="slide.title || slide.description" class="absolute bottom-0 left-0 right-0 p-6 text-white">
            <h3 v-if="slide.title" class="text-2xl font-bold mb-2">{{ slide.title }}</h3>
            <p v-if="slide.description" class="text-gray-200">{{ slide.description }}</p>
          </div>
        </div>
      </div>
    </div>
