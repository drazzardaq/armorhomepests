<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    <div 
      ...
      :key="advisor.id"
      class="bg-white/5 rounded-lg p-6 text-center hover:bg-white/10 transition-colors"
    >
      ...
      <h3 class="text-xl font-bold text-white mb-2">{{ advisor.name }}</h3>
      <p class="text-blue-400 font-medium mb-3">{{ advisor.title }}</p>
      <p class="text-gray-300 text-sm mb-4">{{ advisor.bio }}</p>
      ...
      <div class="flex justify-center space-x-3">
        ...
        >
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9-9a9 9 0 00-9 9m0 0a9 9 0 009-9" />
          </svg>
        </a>
      </div>
    </div>
